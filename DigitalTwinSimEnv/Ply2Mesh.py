#-*- coding: utf-8 -*-
import open3d as o3d

# 1. Read the point cloud
pcd = o3d.io.read_point_cloud("output_point_cloud.ply")

# 2. Estimate normals (for mesh reconstruction)
pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
pcd.orient_normals_consistent_tangent_plane(100)

# 3. Poisson reconstruction
mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pcd, depth=9)

# (Optional) Remove artifacts from low-density areas (for a cleaner result)
import numpy as np
vertices_to_keep = densities > np.quantile(densities, 0.02)
mesh.select_by_index(np.where(vertices_to_keep)[0], cleanup=True)

# 4. Save as .obj or .stl
o3d.io.write_triangle_mesh("output_model1.obj", mesh)
# o3d.io.write_triangle_mesh("output_model.stl", mesh)
print("Successfully exported: output_model1.obj")